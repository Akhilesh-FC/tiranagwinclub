...
public function receive_money(Request $request)
{
 ...
 if(!empty($level_up_reward > 0)){
                        ...
                        DB::table('wallet_history')->insert($data);
                           DB::update("UPDATE `users` SET `wallet` = `wallet` + ? WHERE `id` = ?", [$level_up_reward, $userid]);
                   ...
                   $data = [
                        ...
                        DB::table('wallet_history')->insert($data);
                           $user_id=DB::select("UPDATE `users` SET `wallet`='wallet'+$monthly_rewards WHERE `id`=$userid");
